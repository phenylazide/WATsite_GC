...
//=================================================================================================
void CalcEntropy(water *wtr, hcluster *cluster_0)
{
	...
	if(par.covdimension == 6 || par.covdimension == 9)
	{
		...
		CovarianceMatrix6x6(wtr, cluster_0);
	...
	if(par.covdimension == 3 || par.covdimension == 9)
	{
		...
		CovarianceMatrix3x3(wtr, cluster_0);
...
void EulerAngle(water *wtr, hcluster *cluster_0)
{
	...
	for(cluster = cluster_0, j = 0; j < cluster_0->numclusters; cluster = cluster->next, j++)
	{
		...
		printf("cluster %d: average_occupancy: %f\n", j, cluster->average_occupancy);
...
//====================================================================================================
void CovarianceMatrix6x6(water *wtr, hcluster *cluster_0)
{
	hcluster	*cluster;
	...
	for(cluster = cluster_0, cl = 0; cl < cluster_0->numclusters; cluster = cluster->next, cl++)
	{
		...
		printf("water inside cluster%d: %d (=%d)\n", cl, m, cluster->wtrnum);
...
/*		printf("eigenvectors B:\n");
		...
		//verify calculation by computing new covariance 
		//if it is correct, new covariance matrix should be diagonal matrix where the diagnal elements are 
		//eigenvalues of original covariance matrix
/*		for(m = 0; m < 3; m++)
		{
			for(n = 0; n < 3; n++)
			{
				for(i = 0; i < cluster->wtrnum; i++)
				{
					//cov[m][n] += (coor[i][m] - coor_mean[m])*(coor[i][n] - coor_mean[n]);
					newcov[m][n] += newcoor[m][i]*newcoor[n][i];
					newEulacov[m][n] += newEula[m][i]*newEula[n][i];
				}				
			}
		}
		...
		for(m = 0; m < 3; m++)
		{
			...
			{
				newcov[m][n] /= (cluster->wtrnum - 1);  // ???? n or n-1???
				newEulacov[m][n] /= (cluster->wtrnum - 1);
			}
			
		}	

		printf("new covariance matrix A:\n");
		for(m = 0; m < 3; m++)
			printf("%10.6f %10.6f %10.6f\n", newcov[m][0], newcov[m][1], newcov[m][2]);
		printf("new covariance matrix B:\n");
		for(m = 0; m < 3; m++)
			printf("%10.6f %10.6f %10.6f\n", newEulacov[m][0], newEulacov[m][1], newEulacov[m][2]);
*/
		//free memory
		for(i = 0; i < cluster->wtrnum; i++)
		{
...
{ 
	...
	for(cluster = cluster_0, cl = 0; cl < cluster_0->numclusters; cluster = cluster->next, cl++)
	{
		...
		for(i = 0; i < 6; i++)
		{
			...
			//normalization by simpson's rule
/*			integral = 0.0;
			for(j = 0; j < par.numbins+1; j++)
			{
				if(j == 0)
				{
					p = 0.5*probability[j];
				}
				else if(j == par.numbins)
				{
					p = 0.5*probability[par.numbins-1];
				}
				else
				{
					p = 0.5*(probability[j-1] + probability[j]);
				}
				
				if(j==0 || j==par.numbins)
				{
					integral += p;
				}
				else if(j%2 == 1)
			...
			integral *= interval/3.0;		// unit: kcal/mol//S = -R*integral(p*ln(p))
*/
...
//====================================================================================================
void SimpsonsIntegral(water *wtr, hcluster *cluster_0)
{
	int			i, j, cl;
	...
	for(cluster = cluster_0, cl = 0; cl < cluster_0->numclusters; cluster = cluster->next, cl++)
	{
		...
		/*		if(j == 0)
				...
				{
					p = cluster->probability[i][par.numbins-1];
				}
				else
				{
					p = 0.5*(cluster->probability[i][j-1] + cluster->probability[i][j]);
				}*/
				p = cluster->probability[i][j];
				lnp = log(p);
				if(p < Null)	lnp = 0.0;
