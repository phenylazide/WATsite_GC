...
def Energy_calc(folder_in, folder_out, runscript_file, mpi): 
	...
	for fname in name_lst:
		...
		fout.write(';\n')
                fout.write('cutoff-scheme      = Verlet\n')
		...
		for i in range(0,count+1):		
			...
			comm1 = "$GROMACS_HOME/bin/gmx grompp -f %s -c %s/prot_md.gro -p %s/protein.top -o test.tpr -maxwarn 50 -n %s -quiet\n" % (pr_file,md_output_dir,md_output_dir, ndx_file)
			print comm1
			rfile.write(comm1)

			#make sure the *xtc file is the same one as used to generate the mdsnaps.pdb file
			comm1 = "$GROMACS_HOME/bin/gmx mdrun -ntmpi 16 -rerun %s/prot_md.xtc -s test.tpr -o test.trr -c test.gro -g test.log -e test.edr -quiet\n" % (md_output_dir)
			print comm1
			rfile.write(comm1)
			...
			for j in range(0,num_wtr[i]):
				...
				fin.write("$GROMACS_HOME/bin/gmx energy -f test.edr -s test.tpr -sum -o Energy_output_%d -quiet<< EOF\n" % (wtr_index)) 
				for k in range(0,j):
		...
		for i in range(0,count+1):		
			...
			#make sure the *xtc file is the same one as used to generate the mdsnaps.pdb file
			comm1 = "$GROMACS_HOME/bin/gmx mdrun -rerun %s/prot_md.xtc -s test.tpr -o test.trr -c test.gro -g test.log -e test.edr -quiet\n" % (md_output_dir)
	#		comm1 = "mpirun -np $ncpus $GROMACS_HOME/bin/gmx mdrun_mpi -rerun %s/prot_md.xtc -s test.tpr -o test.trr -c test.gro -g test.log -e test.edr\n" % (md_output_dir)
			print comm1
			rfile.write(comm1)
			rfile.write("rm #*\n")
