...
def main(argv):
	...
	fi = open("protein.top", "r")
	fo = open("protein_lig.top", "w")
	flag = 1
	sol_count = 0
        for line in fi:
                if line.find("SOL") > -1:
	...
	for line in fi:
		if line.find("; Include chain topologies") > -1:
			flag_chain += 1
			fo.write(line)
			fo.write("#include \"ligand_GMX.itp\"\n")
	...
	fi = open("protein_lig.top", "r")
	filelines = fi.readlines()
	fi.close()
	print "\nchain flag"
	print flag_chain
        if flag_chain < 1:
                fi = open("protein_lig.top", "w")
		...
		for i in range(numLine, numLine+2):
                ...
                fi.write('#include \"ligand_GMX.itp\"\n\n')
	...
	os.system("mv protein.top protein_bak.top")
	os.system("cp protein_lig.top protein.top")
main(sys.argv[1:])
