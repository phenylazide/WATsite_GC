...
if ( ! -e prot_em.gro ) then 
  if ( $watermodel != 'tip4pew' ) then
    $GROMACS_HOME/bin/pdb2gmx -ff amber99sb-ildn -f prot.pdb -o protH.pdb -p prot.top -water $watermodel >&/dev/null
  else
    echo '3' | $GROMACS_HOME/bin/pdb2gmx -ff amber99sb-ildn -f prot.pdb -o protH.pdb -p prot.top >&/dev/null
  endif
  ...
  if ( ! -e protH.pdb ) then
    ...
    if ( $watermodel != 'tip4pew' ) then
      $GROMACS_HOME/bin/pdb2gmx -ff amber99sb-ildn -f prot_amber.pdb -o protH.pdb -p prot.top -water $watermodel -ignh -chainsep id >&/dev/null
    else
      echo '3' | $GROMACS_HOME/bin/pdb2gmx -ff amber99sb-ildn -f prot_amber.pdb -o protH.pdb -p prot.top -ignh -chainsep id >&/dev/null
    endif
    sed -i 's/HO4/SOL/g' protH.pdb 
    sed -i 's/HOH/SOL/g' protH.pdb
  endif
...
if ( $withligand == 1) then
  ...
  echo '0' |$GROMACS_HOME/bin/genrestr -f lig_GMX.gro -o posre_lig.itp >&/dev/null

  # fuse ligand and protein files
  mv protH.pdb prot_only.pdb
  $GROMACS_HOME/bin/editconf -f lig_GMX.gro -o lig_GMX.pdb
...
if ( -e protH.pdb ) then
   $GROMACS_HOME/bin/editconf -bt cubic -f protH.pdb -o prot.gro -c -d 1.0 >&/dev/null
   if ( $watermodel == 'spc' || $watermodel == 'spce' ) then
      $GROMACS_HOME/bin/genbox -cp prot.gro -cs spc216.gro -o prot_4ion.gro -p prot.top >&/dev/null
   else if ( $watermodel == 'tip4p' || $watermodel == 'tip4pew' ) then
      $GROMACS_HOME/bin/genbox -cp prot.gro -cs tip4p.gro -o prot_4ion.gro -p prot.top >&/dev/null
   endif
   ...
   $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/em.mdp -c prot_4ion.gro -p prot.top -o prot_4ion.tpr -maxwarn 50 >& charge.log
   #get the ion charge from previous step
   set charge=`python $WATSITEHOME/bin/getCharge.py`
   if ( $charge > 0 ) then
      echo 'SOL' | $GROMACS_HOME/bin/genion -s prot_4ion.tpr -o prot_em.gro -nname CL -nn $charge -p prot.top >&/dev/null
   else if ( $charge < 0 ) then
      set abscharge=`expr $charge \* -1`
      echo 'SOL' | $GROMACS_HOME/bin/genion -s prot_4ion.tpr -o prot_em.gro -nname NA -nn $abscharge -p prot.top >&/dev/null
   else
...
if ($ncpus == 1) then
  ...
  if ( ! -e prot_pr.gro ) then 
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/em.mdp -c prot_em.gro -p prot.top -o prot_min.tpr -maxwarn 50 >&/dev/null
    $GROMACS_HOME/bin/mdrun -s prot_min.tpr -o prot_min.trr -c prot_pr.gro -g min.log -e min.edr
    rm \#*
  endif
  ...
  if ( ! -e prot_equ.gro ) then
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/pr.mdp -c prot_pr.gro -p prot.top -o prot_pr.tpr -maxwarn 50 >&/dev/null
    $GROMACS_HOME/bin/mdrun -s prot_pr.tpr -o prot_pr.trr -c prot_equ.gro -g pr.log -e pr.edr
    rm \#*
  endif
  if ( ! -e prot_md.gro ) then
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/equ.mdp -c prot_equ.gro -p prot.top -o prot_equ.tpr -maxwarn 50 >&/dev/null
    $GROMACS_HOME/bin/mdrun -s prot_equ.tpr -o prot_equ.trr -c prot_md.gro -g equ.log -e equ.edr
    rm \#*
  endif
  if ( ! -e prot_fin.gro ) then
    if ( -e $WATSITEHOME/mdp_files/posre/md_new.mdp ) then
      $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/md_new.mdp -c prot_md.gro -p prot.top -o prot_md.tpr -maxwarn 50 >&/dev/null
    else
      $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/md.mdp -c prot_md.gro -p prot.top -o prot_md.tpr -maxwarn 50 >&/dev/null
    endif
    $GROMACS_HOME/bin/mdrun -s prot_md.tpr -o prot_md.trr -c prot_fin.gro -g md.log -e md.edr
    rm \#*
  endif
  rm \#*
  ...
  if ( ! -e prot_pr.gro ) then 
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/em.mdp -c prot_em.gro -p prot.top -o prot_min.tpr -maxwarn 50 >&/dev/null
    mpirun -np $ncpus $GROMACS_HOME/bin/mdrun_mpi -s prot_min.tpr -o prot_min.trr -c prot_pr.gro -g min.log -e min.edr
    rm \#*
  endif
  # minimization check point
  if ( -e min.log && ! -e prot_pr.gro ) then
    ...
    if ( $exist > 0 ) then
      echo "test3"
      mpirun -np $ncpus $GROMACS_HOME/bin/mdrun_mpi -s prot_min.tpr -o prot_min.trr -c prot_pr.gro -g min.log -e min.edr -nt 1
      rm \#*
    endif
  endif   
 # 3b. md
  if ( ! -e prot_equ.gro ) then
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/pr.mdp -c prot_pr.gro -p prot.top -o prot_pr.tpr -maxwarn 50 >&/dev/null
    mpirun -np $ncpus $GROMACS_HOME/bin/mdrun_mpi -s prot_pr.tpr -o prot_pr.trr -c prot_equ.gro -g pr.log -e pr.edr
    rm \#*
  endif
  if ( ! -e prot_md.gro ) then
    $GROMACS_HOME/bin/grompp -f $WATSITEHOME/mdp_files/posre/equ.mdp -c prot_equ.gro -p prot.top -o prot_equ.tpr -maxwarn 50 >&/dev/null
    mpirun -np $ncpus $GROMACS_HOME/bin/mdrun_mpi -s prot_equ.tpr -o prot_equ.trr -c prot_md.gro -g equ.log -e equ.edr
    rm \#*
  endif
  if ( ! -e prot_fin.gro ) then
    if ( -e $WATSITEHOME/mdp_files/posre/md_new.mdp ) then
    ...
    mpirun -np $ncpus $GROMACS_HOME/bin/mdrun_mpi -s prot_md.tpr -o prot_md.trr -c prot_fin.gro -g md.log -e md.edr
    rm \#*
  endif
  rm \#*
endif
...
if ( -e prot_md.trr && ! -e mdsnaps.pdb ) then
   ...
   echo 'System' | $GROMACS_HOME/bin/trjconv -f prot_md.trr -s prot_md.tpr -o mdsnaps.pdb  >&/dev/null
   $GROMACS_HOME/bin/trjconv -f prot_md.trr -o prot_md.xtc
...
(echo 0; echo 'q';) | $GROMACS_HOME/bin/make_ndx -f prot_em.gro -o index.ndx >&/dev/null
   
# 5. move 
echo 'Protein' | $GROMACS_HOME/bin/trjconv -f prot_md.gro -s prot_md.tpr -o ref.pdb >&/dev/null
