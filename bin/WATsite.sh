...
if ( $withligand == 1) then
	source /home/phzd/test/amber16/amber.sh
...
if ( -e protH.pdb ) then
   ...
   if ( ! -e prot_4ion.gro ) then
      if ( $watermodel == 'spc' || $watermodel == 'spce' ) then
         $GROMACS_HOME/bin/gmx gmx solvate -cp prot.gro -cs spc216.gro -o prot_4ion.gro -p prot.top >&/dev/null
      else if ( $watermodel == 'tip4p' || $watermodel == 'tip4pew' ) then
         $GROMACS_HOME/bin/gmx gmx solvate -cp prot.gro -cs tip4p.gro -o prot_4ion.gro -p prot.top >&/dev/null
      endif
   endif
